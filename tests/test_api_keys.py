...
def test_search_api_key(api_key: str = None, provider: str = None):
    ...
    logger.info(f"Testing search API key for provider: {settings.search_provider.value}...")
...
def main():
    ...
    parser.add_argument(
        "--groq", 
        action="store_true", 
        help="Test Groq API key"
    )
    parser.add_argument(
        "--search", 
        action="store_true", 
        help="Test search API key"
    )
    parser.add_argument(
        "--groq-key", 
        type=str, 
        help="Groq API key to test"
    )
    parser.add_argument(
        "--search-key", 
        type=str, 
        help="Search API key to test"
    )
    parser.add_argument(
        "--model", 
        type=str, 
    ...
    if status["groq"] is not None:
        logger.info(f"Groq API key: {'✓ Working' if status['groq'] else '✗ Not working'}")
    
    if status["search"] is not None:
        logger.info(f"Search API key: {'✓ Working' if status['search'] else '✗ Not working'}")
