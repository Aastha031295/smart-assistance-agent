...
from src.search_engine import SearchResult, search_engine
...
class RAGEngine:
    ...
    def _setup_llm(self):
        ...
        try:
            ...
            return ChatGroq(
                model_name=self.model_name, temperature=0.2, groq_api_key=api_key
            )
    ...
    def setup_rag_chain(
        ...
        kb_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an expert car mechanic assistant that helps identify car parts 
            ...
            earlier in the conversation, make sure to address it appropriately.""",
                ...
                (
                    "system",
                    "Here is information from the knowledge base that may be helpful:\n\n{context}",
                ),
            ]
        )
        ...
        web_prompt = ChatPromptTemplate.from_messages(
            ...
            earlier in the conversation, make sure to address it appropriately.""",
                ...
                (
                    "system",
                    "Here is information from internet search results that may be helpful:\n\n{search_results}",
                ),
            ]
        )
        ...
        kb_chain = (
            {
                "context": lambda x: format_docs(
                    knowledge_base.get_relevant_documents(x["question"])
                ),
